...
Feature: Charge funds from a credit card
  ...
  Scenario: Charge funds from a credit card
    Given I have a credit card with id "e6eb2b4c-94ce-46eb-b01c-67ed491dad26" with available balance "100" GBP
     ...
     Then I should be notified that funds on a credit card were blocked
  ...
  Scenario: Block funds on a not existing credit card
    Given I don't have a credit card
     When I block "100" GBP funds on a credit card with id "6a45032e-738a-48b7-893d-ebdc60d0c3b7"
     Then I should be notified that funds on a credit card does not exist